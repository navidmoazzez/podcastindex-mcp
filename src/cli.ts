 ...
 *
 * The command IS the tool name. `get_transcript` runs as `get-transcript`, and
...
type Flag = {
  /** The schema key, e.g. `episode_id`. */
  key: string;
  /** The long flag, e.g. `--episode-id`. */
  flag: string;
  kind: FlagKind;
  required: boolean;
  repeatable: boolean;
...
function kindOf(schema: ZodTypeAny): { kind: FlagKind; choices?: string[]; repeatable: boolean } {
  ...
  switch (typeName) {
    ...
    case "ZodArray": {
      // An array of scalars is repeatable (`--show a --show b`). An array of
...
export function parseArgs(argv: string[], flags: Flag[]): Record<string, unknown> {
  ...
  // One bare argument fills the first required flag, so `search-podcasts "cats"`
...
/**
 * `--select id,feed.title` keeps only the named fields. Dotted paths descend,
 ...
 *
 * Some tools already return the rendered text from `format/`, which is the
 * shaped-for-a-model output and is just as readable in a terminal. Most return
 * an object. `--json` forces JSON either way so a script never has to know
 * which kind of tool it called.
 */
 ...
 *
 * The package ships two binaries onto the same file. Printing `podcastindex-mcp`
 * at someone who typed `podcastindex-cli` hands them a command that works but is
 * not the one they have in their fingers.
 */
function binName(): string {
  const name = (process.argv[1] ?? "").split("/").pop() ?? "";
  return name.startsWith("podcastindex-cli") ? "podcastindex-cli" : "podcastindex-mcp";
}

/** What a tool reaches out to, said in words rather than the enum's. */
function reaches(spec: AnyToolSpec): string {
  switch (spec.surface) {
    case "index":
      return "the signed Podcast Index API, needs a key and secret";
    case "open":
      return "an open Podcast Index endpoint, no credential needed";
    default:
      return "the podcaster's own host, unvetted and often slow";
  }
}
...
function renderToolHelp(spec: AnyToolSpec): string {
  ...
  lines.push(`Reaches: ${reaches(spec)}`);
  lines.push(``);
  return lines.join("\n");
}
...
export async function runCli(argv: string[]): Promise<number> {
  ...
  if (!spec) {
    ...
    emitError(
      new Error(
        hidden
          ? `${name} is unavailable: this server is running with PODCASTINDEX_READ_ONLY=1.`
